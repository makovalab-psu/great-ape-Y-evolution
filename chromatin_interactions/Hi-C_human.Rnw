...
#tiff(file=paste0(basename(filename),".HiC",".tiff"),units="cm", width=17.9, height=17.9, res=300,compression="rle")
#dev.off()
...
plot(Ypairs$pos1/coef,Ypairs$pos2/coef,xlim=c(0,tlength_without_heterochromatin/coef),ylim=c(0,tlength_without_heterochromatin/coef),xlab="coordinates",ylab="coordinates",col=prob_colors,pch=19,cex=0.2,main=paste("HUMAN from the full mHiC pipeline, 20kb",nrow(Ypairs),"interactions"), sub=myLabel,xaxs="i",yaxs="i",asp=1)

abline(coef = c(0,1),col="darkblue")

#plot bottom down part of the plot
points(Ypairs$pos2/coef,Ypairs$pos1/coef,xlab="coordinates",ylab="coordinates",col=prob_colors,pch=19,cex=0.2,main=paste("HUMAN from the full mHiC pipeline, 20kb",nrow(Ypairs),"interactions"), sub=myLabel)

#highlight PARs and centromere in the plot 
plotRectangle(CENYhg38/coef,adjustcolor("red", alpha.f = 0.5))
...
apply(palindromeshg38,1,function(x) {plotRectangle(x,adjustcolor("brown", alpha.f = 0.1))})
